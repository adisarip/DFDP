...
static ofstream output("OutputRM.txt");
...
void *run_rm(void *data) {
    ...
    unsigned int num_higher_priority = 0;
    for(unsigned int j = 0; j < num_threads; j++) {
        if(j == this_thread_data.id-1) continue;
        if(thread_data_array[j].wcet / thread_data_array[j].period < 
          thread_data_array[this_thread_data.id-1].wcet / thread_data_array[this_thread_data.id-1].period) {
            num_higher_priority++;
        }
    }
    attr.sched_priority = 99 - num_higher_priority;
    
	if(this_thread_data.id == num_threads) {
		usleep((int)(this_thread_data.wcet*.95 * 1000));
	}
	else {
	...
	while(!done) {
		if(this_thread_data.id == 1) {
			...
			if(milliseconds > max_staleness) // && milliseconds < tC_period)
...
int main(int argc, char** argv) {	
	...
	for(i = 1; i <= num_lines; i++) {
        ...
        thread_data_array[num_threads-1].wcet = 1;
		...
		// Create threads
        for(unsigned int j = 0; j < num_threads; j++) {
            pthread_create(&threads[j], NULL, run_rm, (thread_data*)&thread_data_array[j]);
        }
